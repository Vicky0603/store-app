...
public class CartControllerTest {
    ...
    void getCart_createsIfMissing_andAddUpdateRemove() {
        ...
        var principal = new org.springframework.security.core.userdetails.User("u@example.com","x", java.util.List.of());

        when(repo.findByUserEmail("u@example.com")).thenReturn(Optional.empty());
        ...
        Cart c1 = controller.getCart(principal);
        assertEquals(1L, c1.getId());

        var added = controller.addItem(principal, new CartController.AddItemRequest(10L, "Prod", "img", BigDecimal.TEN, 2));
        assertEquals(1, added.getItems().size());
        CartItem item = added.getItems().get(0);
        assertEquals(10L, item.getProductId());

        // update qty
        ResponseEntity<Cart> updated = controller.updateQty(principal, item.getId(), java.util.Map.of("quantity", 3));
        // since id is null before save, update may not find by exact ID; simulate item id set and call again
        item.setId(100L);
        updated = controller.updateQty(principal, 100L, java.util.Map.of("quantity", 3));
        assertTrue(updated.getStatusCode().is2xxSuccessful());

        // remove
        ResponseEntity<?> rem = controller.remove(principal, 100L);
